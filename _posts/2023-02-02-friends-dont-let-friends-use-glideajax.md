...
I know what you're gonna say, this looks like more code than a typical GlideAjax call. That actually depends (my code has pretty robust error handling, for example, and quality GlideAjax really should follow similar best practice), but keep in mind this is _all_ the code. There's no need for you to make any server-side component because the Table <abbr>API</abbr> does all the heavy lifting for you. One Client Script and you're done.
